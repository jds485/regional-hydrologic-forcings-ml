...
screen_daily_data <- function(prescreen_data, year_start = 'water'){
  ...
  if (nrow(prescreen_data) > 0) {
    missing_data <- screen_flow_data(data.frame(site_no = prescreen_data$site_no, 
                                                Date = prescreen_data$Date,
                                                Value = prescreen_data$discharge),
    ...
    if(nrow(complete_yrs) > 0){
      data_out <- data.frame(site_no = unique(prescreen_data$site_no),
